...
let native;
try {
  native = require(path.join("..", "build", "Release", "webrtc_node.node"));
} catch (error) {
  error.message = `${error.message}\nNative addon not found. Run "npm run build" before requiring this package.`;
  throw error;
}
...
class RTCIceTransport extends SimpleEventTarget {
  ...
  getSelectedCandidatePair() {
    ...
    const remote = remoteCandidates[0] || null;
...
class RTCDtlsTransport extends SimpleEventTarget {
  ...
  get state() {
    ...
    if (this._pc.connectionState === "connected") return "connected";
...
class RTCDataChannel extends SimpleEventTarget {
  ...
  send(data) {
    ...
    if (typeof data === "string") {
      ...
      if (this._hasPendingSends()) {
        ...
        this._sendNativeString(payload);
    ...
    if (typeof Blob !== "undefined" && data instanceof Blob) {
      ...
      this._enqueueSend(async () => {
        ...
        this._sendNativeBinary(new Uint8Array(buffer));
      }, blob.size);
      return;
    }
  ...
  _closeNativeAfterBufferedSends(close) {
    ...
    const poll = () => {
      ...
      if ((this._bufferedAmount === 0 && nativeBufferedAmount === 0) || Date.now() >= deadline) {
...
class RTCPeerConnection extends SimpleEventTarget {
  ...
  async createOffer(options = undefined) {
    ...
    try {
      ...
      if (this._iceRestartPending && this._canApplyNativeIceCredentials()) {
        offer = {
          type: offer.type,
  ...
  async setLocalDescription(description = undefined) {
    ...
    try {
      ...
      if (this._localDescription?.type === type && type !== "offer") {
        ...
        const hadIceRestartRequest = this._iceRestartPending && type === "offer";
  ...
  async setRemoteDescription(description) {
    ...
    try {
      ...
      this._ensureNativePeerConnection().setRemoteDescription(remoteDescription.toJSON());
  ...
  _updateSctpTransport() {
    ...
    const nativeConnected =
      this._explicitIceCandidateExchange && this._connectionState === "connected";
    ...
    const connected = (nativeConnected || sameProcessConnected) && operationsSettled;
  ...
  _scheduleDataChannelAnnouncementRepair() {
    ...
    const now = Date.now();
    if (!this._dataChannelAnnouncementRepairDeadline) {
      this._dataChannelAnnouncementRepairDeadline =
        now + DATA_CHANNEL_ANNOUNCEMENT_REPAIR_TIMEOUT_MS;
    }
    if (now >= this._dataChannelAnnouncementRepairDeadline) {
      this._dataChannelAnnouncementRepairDeadline = 0;
      return;
    }
    ...
    setTimeout(() => {
      this._dataChannelAnnouncementRepairScheduled = false;
      if (this._closed) return;
      const transportsConnected =
        this._sctpTransport?.state === "connected" &&
        this._pairedPeer?._sctpTransport?.state === "connected";
      const hasWork = transportsConnected
  ...
  _currentSctpMaxChannels() {
    ...
    if (!this._explicitIceCandidateExchange || !connected) return null;
  ...
  _canApplyNativeIceCredentials() {
    return this._iceGatheringState === "new";
  }
  ...
  _handleNativeEvent(event) {
    ...
    if (event.target === "datachannel") {
      ...
      if (channel) channel._handleNativeEvent(event);
      return;
    }

    switch (event.type) {
      case "datachannel": {
        const channel = RTCDataChannel._fromNative(this, event.channel, event.channelReadyState);
        const id = channel.id;
        if (id != null) {
          const synthetic = this._pendingSyntheticDataChannelAnnouncements.get(id);
          if (synthetic && synthetic.readyState !== "closed") synthetic.close();
          this._pendingSyntheticDataChannelAnnouncements.delete(id);
