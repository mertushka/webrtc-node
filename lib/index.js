...
class RTCDataChannel extends SimpleEventTarget {
  ...
  constructor(peerConnection, nativeChannel, initialReadyState = undefined, assignedId = null) {
    ...
    this._nativeFastSend =
      typeof nativeChannel.sendStringFast === "function" &&
      typeof nativeChannel.sendBinaryFast === "function";
    this._readyState =
      initialReadyState ||
  ...
  _sendNativeString(data) {
    ...
    try {
      let sent = this._nativeFastSend
        ? this._native.sendStringFast(data)
  ...
  _sendNativeBinary(data) {
    ...
    try {
      const view = data instanceof Uint8Array ? data : toUint8Array(data);
      let sent = this._nativeFastSend
        ? this._native.sendBinaryFast(view)
        : this._native.sendBinary(view);
      if (sent !== false) {
        sent = true;
      } else {
        sent = Boolean(this._native?.isOpen);
  ...
  _dispatchNativeMessageBatch(events) {
    ...
    for (let index = 0; index < events.length; index += 1) {
      ...
      const messageEvent = makeMessageEvent("message", {
        data:
          event.binary && this._binaryType === "blob" && typeof Blob !== "undefined"
            ? new Blob([event.data])
            : event.data,
      });
      messageEvent.target = this;
