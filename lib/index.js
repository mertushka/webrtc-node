...
class RTCDataChannel extends SimpleEventTarget {
  ...
  _hasMessageEventListeners() {
    const listeners = kHandlers.get(this)?.get("message");
    return Boolean(listeners?.length);
  }

  _canContinueMessageBatchInCurrentTask(handler, yieldToTask, hasMessageListeners) {
    return (
  ...
  _canUseDirectMessageHandler(handler, yieldToTask, hasMessageListeners) {
    return this._canContinueMessageBatchInCurrentTask(handler, yieldToTask, hasMessageListeners);
  }
  ...
  _flushQueuedMessageEvents() {
    while (true) {
      ...
      const yieldToTask = this._messageContinuationNeedsTaskYield(handler);
      ...
      if (!this._canContinueMessageBatchInCurrentTask(handler, yieldToTask, hasMessageListeners)) {
  ...
  _ensureNativePeerConnection() {
    ...
    const nativePeerConnection = new native.NativePeerConnection(nativeConfiguration, (event) => {
      if (Array.isArray(event)) {
        for (const item of event) this._handleNativeEvent(item);
      } else {
